...
reference implementation evaluated against synthetic ground truth and two open clinical datasets
at different sampling rates (PhysioNet CHB-MIT, 256\,Hz; Sleep-EDF, 100\,Hz), we establish three
results. First, filtering display windows
independently produces chunk-seam artifacts; an overlap-add scheme that filters an extended
window and keeps the clean center removes them by about 35\,dB on synthetic data and 23--51\,dB
across two real datasets, at negligible latency. Second, zero-phase forward--backward filtering (\texttt{filtfilt})
...
\mathrm{taps}$, and confirm both across both datasets. Third, in windowed spectral
...
explicit: the same 606-sample floor is \textbf{6.06}\,s at 100\,Hz (vs.\ 2.37\,s at
256\,Hz), and the fallback mis-times events by $\sim$1\,s rather than $\sim$0.4\,s---the lower the
sampling rate, the longer both the floor and the mis-timing.
